...
int main(){
    ...
    for (int i; i < stagger_and_step_num; i++){
        ...
        // otherwise, try stagger and step in parallel
            ...
            */
            ...
            #pragma omp parallel for num_threads(numThreads) schedule(dynamic) shared(success, max_duration, CR, counter, max_perturbation, original_x_0, numThreads) firstprivate(num_variables, check_steps, progress_steps, perturb_min, perturb_max)
            for (int j = 0; j < limit; j++){
                ...
                for (int k = 0; k < check_steps; k++){
                    Local_now = CR.rk4(Local_now); // integrate
                    Local_now_time += Local_CR.dt;
                    if (isLaminar(Local_now)){
                        if (k < progress_steps){
    ...
    */
    ...
    oss << "../../generated_lam_img/gen_laminar_epsilon" << params.epsilon << "_t" << t << "_a" << params.a << "_c" << params.c << "_f" << params.f << "_omega" << params.omega1 << "-" << params.omega2  << "_" << check << "check" << progress << "progress10^" << logged_min_perturbation<<"-10^"<< logged_max_perturbation << "perturb.png";
    ...
    */
    ...
    if(calced_laminar.cols() > 1){
        if (progress == t){
            ...
            oss << "../../generated_lam/gen_laminar_epsilon" << params.epsilon << "_t" << t << "_a" << params.a << "_c" << params.c << "_f" << params.f << "_omega" << params.omega1 << "-" << params.omega2  << "_" << check << "check" << progress << "progress10^" << logged_min_perturbation<<"-10^"<< logged_max_perturbation << "perturb.npy";
            std::string fname = oss.str(); // 文字列を取得する
            std::cout << "saving as " << fname << std::endl;
...
bool isLaminar(Eigen::VectorXd state){
    if (state(4) > state(0) - 15 && state(4) < state(0) + 15){
        return true;
    }
    else{
        return false;
